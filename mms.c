...
#include "mms_thread.c"

mms_t *mms_connect (mms_io_t *io, void *data, const char *url, int bandwidth) {
  mms_thrd_args mms_conn_args;
  pthread_t mms_conn_thrd;
  
  memset( &mms_conn_args, 0, sizeof(mms_conn_args) );
  mms_conn_args.io = io;
  mms_conn_args.data = data;
  mms_conn_args.url = url;
  mms_conn_args.bandwidth = bandwidth;
  
  mms_conn_flag = NULL;
  signal(SIGALRM, mms_alarm);
  if ( pthread_create(&mms_conn_thrd, 0, mms_conn_thread, 
	  &mms_conn_args) != 0 ) {
	fprintf( stderr, "mmsh_connect_int create thread error\n" );
	return 0;
  }
  pthread_join(mms_conn_thrd, NULL);
  
  return mms_conn_flag;
}
