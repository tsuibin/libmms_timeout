...
#include "mmsh_thread.c"

static int mmsh_connect_int (mms_io_t *io, mmsh_t *this, off_t seek, uint32_t time_seek) {
  mmsh_thrd_args conn_init_args;
  pthread_t conn_init_thrd;
  
  memset( &conn_init_args, 0, sizeof(conn_init_args) );
  conn_init_args.io = io;
  conn_init_args.this = this;
  conn_init_args.seek = seek;
  conn_init_args.time_seek = time_seek;
  
  conn_init_flag = 0;
  signal(SIGALRM, mmsh_alarm);
  if ( pthread_create(&conn_init_thrd, 0, mmsh_conn_thread, 
	  &conn_init_args) != 0 ) {
	fprintf( stderr, "mmsh_connect_int create thread error\n" );
	return 0;
  }
  pthread_join(conn_init_thrd, NULL);
  
  return conn_init_flag;
}
/*
 * end changed
 */
